...
class CAS_Client {
            ...
            if( $this->hasTicket( ) ) {
                switch ($this->getServerVersion()) {
                    case CAS_VERSION_1_0 :
                        ...
                        $this->validateCAS10( $validate_url, $text_response, $tree_response );
                    ...
                    case CAS_VERSION_2_0 :
                        ...
                        $this->validateCAS20( $validate_url, $text_response, $tree_response );
    ...
    public function logout( $params ) {
        ...
        if( isset( $params['service'] ) ) {
            ...
            $cas_url = $cas_url . $paramSeparator . "service=" . urlencode( $this->getURL( ) );
        }
        session_write_close( );
        header( 'Location: ' . $cas_url );
    ...
    public function validateCAS10( &$validate_url, &$text_response, &$tree_response ) {
        ...
        if( !$this->_readURL( $validate_url, $headers, $text_response, $err_msg ) ) {
            ...
            throw new CAS_AuthenticationException( 'CAS 1.0 ticket not validated' );
            $result = false;
        }

        if( preg_match( '/^no\n/', $text_response ) ) {
            phpCAS::trace( 'Ticket has not been validated' );
            throw new CAS_AuthenticationException( 'ST not validated' );
            $result = false;
        } else if( !preg_match( '/^yes\n/', $text_response ) ) {
            phpCAS::trace( 'ill-formed response' );
            throw new CAS_AuthenticationException( 'Ticket not validated' );
            $result = false;
        }
    ...
    public function validateSA( &$validate_url, &$text_response, &$tree_response ) {
        ...
        if( !$this->_readURL( $validate_url, $headers, $text_response, $err_msg ) ) {
            phpCAS::trace( 'could not open URL \'' . $validate_url . '\' to validate (' . $err_msg . ')' );
            throw new CAS_AuthenticationException( 'SA not validated' );
        }
    ...
    private function _validatePGT( &$validate_url, $text_response, $tree_response ) {
        ...
        if( $tree_response->getElementsByTagName( "proxyGrantingTicket" )->length == 0 ) {
            ...
            throw new CAS_AuthenticationException( 'Ticket validated but no PGT Iou transmitted' );
        } else {
            ...
            if( preg_match( '/PGTIOU-[\.\-\w]/', $pgt_iou ) ) {
                ...
                if( $pgt == false ) {
                    ...
                    throw new CAS_AuthenticationException( 'PGT Iou was transmitted but PGT could not be retrieved' );
                }
                $this->_setPGT( $pgt );
            } else {
                phpCAS::trace( 'PGTiou format error' );
                throw new CAS_AuthenticationException( 'PGT Iou was transmitted but has wrong format' );
            }
        }
        phpCAS::traceEnd( true );
        return true;
    ...
    public function serviceWeb( $url, &$err_code, &$output ) {
        ...
        try {
            ...
            $output = sprintf( "The service '%s' is not available (%s).", $url, $e->getMessage( ) );
    ...
    public function serviceMail( $url, $serviceUrl, $flags, &$err_code, &$err_msg, &$pt ) {
        ...
        try {
            ...
            $lang = $this->getLangObj( );
            $err_msg = sprintf( "The service '%s' is not available (%s).", $url, $e->getMessage( ) );
            $err_code = PHPCAS_SERVICE_NOT_AVAILABLE;
            $pt = false;
            return false;
        }
        ...
        if( !($dom->loadXML( $text_response )) ) {
            ...
            if( $success_elements->item( 0 )->getElementsByTagName( "user" )->length == 0 ) {
                ...
                if( !$this->getAllowedProxyChains( )->isProxyListAllowed( $proxyList ) ) {
                    throw new CAS_AuthenticationException( 'Proxy not allowed' );
                    $result = false;
                } else {
                    $result = true;
                }
